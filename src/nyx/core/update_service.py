    ...
    async def _check_on_startup(self):
        ...
        try:
            ...
            if update_info:
                ...
                try:
                    from nyx.gui.update_notifications import show_update_notification
                    # Only show notification if GUI is available
                    show_update_notification(update_info)
                except ImportError:
                    # GUI not available, log only
                    logger.info(f"Update available: {update_info.get('version')} - {update_info.get('changelog', '')[:100]}")
